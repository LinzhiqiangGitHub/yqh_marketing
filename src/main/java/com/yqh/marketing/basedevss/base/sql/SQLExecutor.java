...
import com.yqh.marketing.basedevss.base.data.RecordHandler;
...
import com.yqh.marketing.basedevss.base.util.ObjectHolder;
import com.yqh.marketing.basedevss.base.util.RandomUtils;
import com.yqh.marketing.common.cache.SpyMemcachedUtil;
...
import static com.yqh.marketing.basedevss.base.sfs.SFSUtils.saveUploadTempFile;
...
public class SQLExecutor {
    ...
    public static Record bindRecord(ResultSet rs, Record rec) {
        try {
            ...
            for (String col : cols) {
                if (col.contains("_TIME") || col.contains("STATUSDATE") || col.contains("_time")) {
                    try {
                        Timestamp ts = rs.getTimestamp(col);
                        if (String.valueOf(ts).length()>=19){
                            rec0.put(col, String.valueOf(ts).substring(0,19));
                        }  else {
                            rec0.put(col,String.valueOf(ts));
                        }
                    } catch (Exception e) {
                        rec0.put(col, "");
                    }
        ...
        try {
            ...
            for (String col : cols) {
                ...
                if (col.equals("NEWS_CONTENT111") || col.equals("CONTENT111")) {
                    rec0.put(col, Clob2String((Clob) o));
                } else {
                    rec0.put(col, o);
                }
            }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                */
                ...
                Blob blob = (Blob) rs.getBlob("ATTACH_BLOB");
