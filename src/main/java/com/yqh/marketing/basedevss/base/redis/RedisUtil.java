package com.yqh.marketing.basedevss.base.redis;
...
public class RedisUtil {
    private static String ip = GlobalConfig.get().getString("jedis.auth.server","127.0.0.1");
    private static String password = GlobalConfig.get().getString("jedis.auth.password","");
    private static int port = (int)GlobalConfig.get().getInt("jedis.auth.server.port",6379);
    private static int maxActive = 300;  //最大连接数
    private static int maxIdle = 200;
    private static long maxWait = 3;
    ...
    public RedisUtil() {
        if (pool == null) {
            ...
            if ((int) GlobalConfig.get().getInt("jedis.auth", 0) == 1)
                pool = new JedisPool(config, ip, port, 3000, password);
            else
