package com.yqh.marketing.basedevss.base.util.image;
...
public class ImageUtils {
    ...
    public static BufferedImage scale(BufferedImage image, String width, String height) {
        ...
        BufferedImage dst0 = new BufferedImage(dstCM, dstCM.createCompatibleWritableRaster( Integer.valueOf(width), Integer.valueOf(height) ),dstCM.isAlphaPremultiplied(), null);
        Image scaleImage = image.getScaledInstance( Integer.valueOf(width), Integer.valueOf(height), Image.SCALE_SMOOTH );

        //BufferedImage dst = new BufferedImage(dstWidth, dstHeight, BufferedImage.TYPE_INT_RGB);
        Graphics2D g2 = null;
        try {
            g2 = dst0.createGraphics();
            //g2.drawImage(scaleImage, transform, null);
            g2.drawImage( scaleImage, 0, 0, Integer.valueOf(width), Integer.valueOf(height), null );
            return dst0;
        } finally {
            if (g2 != null)
                g2.dispose();
    ...
    public static BufferedImage scaleCut(BufferedImage image, int beginX,int beginY,String width, String height) {
        Validate.notNull(image);
        ...
        try {
            g2 = dst0.createGraphics();
            g2.drawImage( scaleImage, beginX, beginY, Integer.valueOf(width), Integer.valueOf(height), null );
            return dst0;
        } finally {
            if (g2 != null)
                g2.dispose();
    ...
    public static void scaleNewCut(InputStream in, OutputStream out, int beginX,int beginY,String width, String height, String format) throws IOException {
        Validate.notNull(in);
        Validate.notNull(out);
        Validate.notNull(format);
        try {
            BufferedImage img = scaleCut(ImageIO.read(in),beginX,beginY, width, height);
