...
//这里用新的表头的样式
    ...
    public byte[] createInboundPackages(String TITLE,String sheetName,List datas) {
        try {
            ByteArrayOutputStream os = new ByteArrayOutputStream();//输出流指定文件路径
    ...
    public byte[] createTurnTableLuckyUser(String TITLE,String sheetName,List datas) {
        try {
            ByteArrayOutputStream os = new ByteArrayOutputStream();//输出流指定文件路径
            ...
            sheet = workbook.createSheet(sheetName, 0); //添加第一个工作表
            sheet.getSettings().setShowGridLines(true);
            initialSheetSetting(sheet,13);//初始化表格属性(公共方法)
    ...
    public byte[] createOrderExcel(String TITLE,String sheetName,List datas) {
        try {
            ByteArrayOutputStream os = new ByteArrayOutputStream();//输出流指定文件路径
            ...
            sheet.setColumnView( 15 , 56 );

            sheet.setColumnView( 16 , 12 );
            sheet.setColumnView( 17 , 14 );

            sheet.setColumnView( 18 , 24 );
            sheet.setColumnView( 19 , 120 );
    ...
    public byte[] createSoldProductsExcel(String TITLE,String sheetName,List datas) {
        try {
            ByteArrayOutputStream os = new ByteArrayOutputStream();//输出流指定文件路径
            ...
            for (int i = 0; i < datas.size()-1; i++) {
                List data = (List) datas.get(i);
                insertRowData(sheet, i + 1, data, getDataCellFormat(CellType.STRING_FORMULA,10,Colour.BLACK,false));
            }
            //插入最后一行，加粗的
            List data_last = (List) datas.get(datas.size()-1);
            insertRowData(sheet, datas.size(), data_last, getDataCellFormat(CellType.STRING_FORMULA,11,Colour.ICE_BLUE,true));
    ...
    public byte[] createGoodsInfoExcel(String TITLE,String sheetName,List datas) {
        try {
            ByteArrayOutputStream os = new ByteArrayOutputStream();//输出流指定文件路径
