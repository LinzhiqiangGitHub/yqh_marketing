...
public class StringUtil {
    ...
    public static boolean isEmptyDisable(String s) {
        if (s != null && !s.equals("") && !s.equals("undefined") && !s.equalsIgnoreCase("null")) {
            return false;
        }
        return true;
    }
    ...
    public static String strRandom(int bit) {
        if (bit == 0)
            bit = 6; // 默认6位
        String str = "";
        str = "abcdefghijkmnpqrstuvwxyz";// 初始化种子
        return RandomStringUtils.random(bit, str);// 返回6位的字符串
    }
    ...
    public static String mobileHideChar(String moblie,int start,int len)
    {
        if(start<1){
            start=1;
        }
        if(isNotEmpty(moblie)){
            char[] charArr = moblie.toCharArray();
            for (int i = 0; i < charArr.length; i++) {
              if(i>=start-1 && i<start+len-1)
                  charArr[i]='*';
            }
            moblie = String.valueOf(charArr);
        }
        return moblie;
    }
    
    public static String replaceIP(String ip){
  	  String[] ips = ip.split("\\.");
  	  ip = "";
